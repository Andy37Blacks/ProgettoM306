...
import { getDatabase, ref, push, set, onChildAdded, orderByChild, query, remove } from "https://www.gstatic.com/firebasejs/10.14.0/firebase-database.js";
...
function loadMessages() {
    const messagesRef = query(ref(database, 'message/'), orderByChild('timeUnix'));  // Ordina i messaggi per timeUnix
    
    onChildAdded(messagesRef, (data) => {
        const messageData = data.val();
        const messageKey = data.key;

        // Ottieni la data attuale e calcola la differenza in millisecondi
        const currentTime = Date.now();
        const messageTime = messageData.timeUnix;
        const oneWeekInMillis = 7 * 24 * 60 * 60 * 1000;

        if (currentTime - messageTime > oneWeekInMillis) {
            // Rimuove il messaggio dal database
            remove(ref(database, 'message/' + messageKey))
                .catch((error) => {
                    console.error('Errore nella rimozione del messaggio:', error);
                });
            return;
        }
        ...
        // Cambia il nome della classe in base all'autore del messaggio
        ...
        this.on("addedfile", (file) => {
            ...
            reader.onload = (e) => {
                if (file.status == 'uploading') {
                    ...
                    alert("File non valido o troppi file aggiunti!!!");
                }
            };
